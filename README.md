...
> Notes pratiques (robustesse) :
> - les labels peuvent être fournis via `label_fields` (ordre de priorité) et les clés de `label_map` sont **normalisées** (lowercase + `_`) avant lookup ;
> - les configs sklearn acceptent `ngram_range` en liste/str et sont converties en tuple, avec un `min_df` auto-clampé pour les jeux minuscules ;
> - l'évaluation spaCy charge automatiquement `model-best/` ou `model-last/` si présents dans le dossier du modèle.
